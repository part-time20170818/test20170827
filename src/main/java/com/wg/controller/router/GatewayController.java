...
public class GatewayController {
    ...
    public Object gateway(HttpServletRequest request, GatewayRequestBO requestBO,BindingResult bindingResult, HttpServletResponse response) throws ServletException, IOException {
       ...
       try{
           dataMap = (Map<String, Object>) JSONObject.parse(requestBO.getData());
       }catch (Exception e){
           e.printStackTrace();
       }
        dataMap.put("service",requestBO.getService());
        dataMap.put("userId",requestBO.getData());
        try {
     ...
     * @throws Exception
     */
    private Object doGateway(Map<String, Object> dataMap) throws Exception {
        String serviceName = (String)dataMap.get(GatewayRequestBO.SERVICE_NAME);
